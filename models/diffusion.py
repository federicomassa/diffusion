...
class DiffusionModel:
    ...
    def _sample_step(self, x_t: tf.Tensor, t: int) -> tf.Tensor:
        ...
        alpha_cumprod_prev = tf.constant(
            self.alphas_cumprod[t - 1] if t > 0 else 1.0, dtype=tf.float32
        )
    ...
    def train_step(
        self, x_0: tf.Tensor, optimizer: tf.keras.optimizers.Optimizer
    ) -> Dict[str, float]:
        """
        ...
        t = tf.random.uniform(
            shape=(batch_size, 1), minval=0, maxval=self.time_steps, dtype=tf.int32
        )

        with tf.GradientTape() as tape:
            # Diffuse the data
            alphas_cumprod_t = tf.gather(
                tf.constant(self.alphas_cumprod, dtype=tf.float32), t
            )
            ...
            x_t = (
                tf.sqrt(alphas_cumprod_t) * x_0
                + tf.sqrt(1.0 - alphas_cumprod_t) * noise
            )
        ...
        for epoch in range(epochs):
            ...
            for step in range(steps_per_epoch):
                ...
                step_result = self.train_step(
                    tf.convert_to_tensor(x_batch, dtype=tf.float32), optimizer
                )
