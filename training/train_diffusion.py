...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train a diffusion model on multimodal Gaussian data")
...
def generate_data(args: argparse.Namespace) -> Tuple[np.ndarray, np.ndarray]:
    ...
    print(f"Generating {args.n_samples} samples from multimodal Gaussian distribution...")
...
def visualize_diffusion_process(args: argparse.Namespace, model: DiffusionModel) -> None:
    ...
    timesteps = [min(t, model.time_steps - 1) for t in range(0, model.time_steps, step_size)]
